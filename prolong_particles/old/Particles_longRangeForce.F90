...
!!  
!! CTSS - June 2012, modified for use with Smooth_particles module
!!***
...
subroutine Particles_longRangeForce (particles,p_count,mapType)
  ...
  use Smooth_particles, ONLY : GetSmoothParticleAcceleration
...
!-------------------------------------------------------------------------
  ...
  integer :: blockCount, i, b
  integer :: numAttrib, ierr
  integer,dimension(2,1) :: attrib
  integer :: num_smooth_save, num_smooth_global
  logical :: doingSmoothing
...
! Map gravitational acceleration to particle positions
  ...
  ! Handle the acceleration of smooth particles:
  if(SmoothParticles) then
     call GetSmoothParticleAcceleration()
  else 
     ! Making sure...
