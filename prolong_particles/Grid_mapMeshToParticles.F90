...
subroutine Grid_mapMeshToParticles (particles, part_props,part_blkID,&
  ...
  use Smooth_data, ONLY : dont_skip_smoothed_particles
...
#include "Particles.h"
  ...
  if (dont_skip_smoothed_particles) then
    smooth_part_prop = 1
  else
  ...
  if(numParticles>0) then
     ...
     do i = 1, numParticles
        ...
        ! skip smoothed particles during normal operation
        ! but not when getting accelerations
        if((particles(smooth_part_prop,i) .eq. 1.0) .and. (.not. dont_skip_smoothed_particles))  then
          cycle
        end if
