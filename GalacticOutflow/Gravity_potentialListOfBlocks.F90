...
subroutine Gravity_potentialListOfBlocks(blockCount,blockList, potentialIndex)
  ...
  use Grid_interface, ONLY : GRID_PDE_BND_PERIODIC, GRID_PDE_BND_NEUMANN, &
       ...
       Grid_solvePoisson
  use Smooth_particles, ONLY : SmoothParticleDensity
  use smooth_Data, ONLY : SmoothParticles
  implicit none
...
#include "Flash_mpi.h"
  ...
  scaleFactor = csm_scaleFactor
  oldScaleFactor = csm_oldScaleFactor
...
!=========================================================================
  ...
  if (grv_meshComm.eq.MASTER_PE) print *,'DEBUG'
...
! This only gets called if there are active particles.
  ...
  if(SmoothParticles) then
     call SmoothParticleDensity(PDE_VAR)
     density = PDE_VAR
  end if
#endif
...
! Un-junk PDEN if it exists and if requested.
  ...
  if (grav_unjunkPden) then
...
#ifdef PDE_VAR
     if (SmoothParticles) density = PDE_VAR
#endif
     ...
     do lb = 1, blockCount
        ...
        call Grid_releaseBlkPtr(blocklist(lb), solnVec)
     enddo
     if (density .NE. PDEN_VAR .and. density .NE. PDE_VAR) call Grid_notifySolnDataUpdate( (/density/) )
#endif
  end if
#endif
